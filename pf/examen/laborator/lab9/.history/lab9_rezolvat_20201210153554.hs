  ...
  insert k v tree = fst (insert' k v tree)
    where
      ...
      go ((less_than_key, OverLimit): list') =
        ...
        in if e then let [(less_than_key'', element'),(greater_than_key'', OverLimit)] = less_than_key' in [(less_than_key'', element'), (greater_than_key'',OverLimit)]
            else (less_than_key', OverLimit): list'
      go ((less_than_key, Element key o): list')
        | k == key = (less_than_key, Element key (Just v)): list'
        | k < key = 
          let (less_than_key', e) = insert' k v less_than_key
          in if e then let [(less_than_key'', element'),(greater_than_key'', OverLimit)] = less_than_key' in (less_than_key'', element'): (greater_than_key'',Element key o): list'
