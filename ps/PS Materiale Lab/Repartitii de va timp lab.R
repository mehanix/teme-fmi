...
#primul argument reprezinta vectorul de valori in care vrem sa evaluam functia
#dgeom(x,p)
#dbinom(x,n,p)
# Ce inseamna?
...
#3. r+nume_repartitie=genereaza valori din acel tip de repartitie
 #  rbinom(nr,n,p)
   ...
   rbinom(3,5,0.4) # genereaza 3 valori dintr-o v.a. repratizata binomial cu parametrii 5 si 0.4
   y <- rbinom(10^6,5,0.4)
   hist(y)
   ...
   # daca generez un esantion ic, potrivirea nu mai e la fel de specataculoasa
   y2 <- rexp(100,1)
   hist(y2, freq=F)
   t <- seq(0.001,8,0.001)
   lines(t, dexp(t,1), col="magenta")
